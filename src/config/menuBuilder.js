...
export const buildMenuFromApi = (apiPermissions) => {
  ...
  const menu = apiPermissions
    .filter((module) => {
      ...
      const isTraspaso = module.nombreOpcion
        ?.toLowerCase()
        .includes("traspaso");
      const hasSubOptions =
        (module.subOpcionesMenu && module.subOpcionesMenu.length > 0) ||
        isTraspaso;

      if (!hasSubOptions && !isTraspaso) {
        console.log(
          `⚠️ Módulo "${module.nombreOpcion}" sin subopciones, se omite`,
        );
      }
      ...
      if (module.subOpcionesMenu) {
        menuItem.subElementos = module.subOpcionesMenu
          ...
          .map((subOption) => {
            ...
            console.log(
              `  └─ Subopción: ${subOption.nombreOpcion} → ${subRoute}`,
            );
    ...
    .sort((a, b) => {
      ...
      const orderA =
        apiPermissions.find((m) => m.nombreOpcion === a.texto)?.orden || 999;
      const orderB =
        apiPermissions.find((m) => m.nombreOpcion === b.texto)?.orden || 999;
...
export const getFilteredMenuItems = (apiPermissions) => {
  console.log(
    "🔄 getFilteredMenuItems llamado con:",
