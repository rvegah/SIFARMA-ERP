...
const PurchaseItemsTable = ({
    ...
    const handleSaveEdit = useCallback((id) => {
        Object.keys(editValues).forEach(field => {
            const val = ["cantidad", "costoUnitario", "precioUnitario"].includes(field)
                ? (editValues[field] === "" ? 0 : Number(editValues[field]))
                : editValues[field];
            ...
            >
                <Table size="small" stickyHeader>
                    ...
                    <TableBody>
                        {/* FILA DE BÚSQUEDA */}
                        {!isFinished && (
                            <TableRow sx={{ position: "sticky", top: 56, zIndex: 1100, bgcolor: "white" }}>
                                <TableCell
                                    colSpan={8}
                                    sx={{ p: 1, borderBottom: `2px solid ${farmaColors.primary}`, bgcolor: "white" }}
                                >
                                    <TextField
                                        fullWidth
                                        inputRef={(ref) => {
                                        ...
                                        InputProps={{
                                            startAdornment: (
                                                <InputAdornment position="start">
                                                    {isSearching
                                                        ? <CircularProgress size={18} sx={{ color: farmaColors.primary }} />
                                        ...
                                        sx={{
                                            "& .MuiOutlinedInput-root": {
                                                bgcolor: "white",
                                                fontSize: "0.9rem",
                                                "& fieldset": { borderColor: farmaColors.alpha?.primary30 || "rgba(204,108,6,0.3)" },
                        ...
                        {items.map((item, index) => {
                            ...
                            return (
                                <TableRow
                                    ...
                                    sx={{
                                        bgcolor: "white",
                                        "&:hover": { bgcolor: farmaColors.alpha?.primary10 || "rgba(204,108,6,0.05)" },
                                    }}
                                >
                                    {/* PRODUCTO */}
                                    <TableCell>
                                        <Typography variant="body2" sx={{ fontWeight: 600, fontSize: "0.82rem", color: farmaColors.secondary }}>
                                            {item.nombreProducto}
                                        </Typography>
                                        <Typography variant="caption" color="text.secondary" sx={{ fontSize: "0.7rem" }}>
                                            {item.codigoProducto} | {item.presentacion}
                                        </Typography>
                                        {item.laboratorio && (
                                    ...
                                    <TableCell>
                                        {isEditing ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setEditValues(prev => ({ ...prev, numeroLote: e.target.value }))}
                                                sx={{ width: 90 }}
                                    ...
                                    <TableCell>
                                        {isEditing ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setEditValues(prev => ({ ...prev, fechaVencimiento: e.target.value }))}
                                    ...
                                    <TableCell align="center">
                                        {isEditing ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setEditValues(prev => ({ ...prev, cantidad: e.target.value }))}
                                                inputRef={cantidadInputRef}
                                                inputProps={{ min: 1, step: 1, style: { fontSize: "0.8rem", textAlign: "center" } }}
                                                sx={{ width: 70 }}
                                            />
                                        ) : (
                                    ...
                                    <TableCell align="center">
                                        {isEditing ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setEditValues(prev => ({ ...prev, costoUnitario: e.target.value }))}
                                                inputProps={{ min: 0, step: 0.01, style: { fontSize: "0.8rem", textAlign: "center" } }}
                                                sx={{ width: 85 }}
                                            />
                                        ) : (
                                    ...
                                    <TableCell align="center">
                                        {isEditing ? (
                                            <TextField
                                                ...
                                                onChange={(e) => setEditValues(prev => ({ ...prev, precioUnitario: e.target.value }))}
                                                inputProps={{ min: 0, step: 0.01, style: { fontSize: "0.8rem", textAlign: "center" } }}
                                                sx={{ width: 85 }}
                                            />
                                        ) : (
                                    ...
                                    <TableCell align="right">
                                        <Typography variant="body1" sx={{ fontWeight: 700, color: farmaColors.primary, fontSize: "0.88rem" }}>
                                            {total.toFixed(2)}
                                        </Typography>
                                    </TableCell>
                                                ...
                                                <>
                                                    <Tooltip title="Guardar (Enter)">
                                                        <IconButton
                                                            ...
                                                            sx={{ bgcolor: "#4CAF50", color: "white", width: 26, height: 26, "&:hover": { bgcolor: "#388e3c" } }}
                                                        >
                                                    ...
                                                    <Tooltip title="Cancelar (Esc)">
                                                        <IconButton
                                                            ...
                                                            sx={{ bgcolor: "#f44336", color: "white", width: 26, height: 26, "&:hover": { bgcolor: "#d32f2f" } }}
                                                        >
                                                        ...
                                                        <>
                                                            <Tooltip title="Editar (A)">
                                                                <IconButton
                                                                    ...
                                                                    sx={{ bgcolor: "#4CAF50", color: "white", width: 26, height: 26, "&:hover": { bgcolor: "#388e3c" } }}
                                                                >
                                                                    <Typography sx={{ fontWeight: 700, fontSize: "0.72rem" }}>A</Typography>
                                                                </IconButton>
                                                            </Tooltip>
                                                            <Tooltip title="Eliminar (E)">
                                                                <IconButton
                                                                    ...
                                                                    sx={{ bgcolor: "#f44336", color: "white", width: 26, height: 26, "&:hover": { bgcolor: "#d32f2f" } }}
                                                                >
                                                                    <Typography sx={{ fontWeight: 700, fontSize: "0.72rem" }}>E</Typography>
                                                                </IconButton>
                                                            </Tooltip>
                                                        </>
                                                    )}
                        ...
                        {items.length === 0 && (
                            <TableRow>
                                <TableCell colSpan={8} sx={{ textAlign: "center", py: 6 }}>
                                    <InventoryIcon sx={{ fontSize: 40, color: "text.disabled", mb: 1, opacity: 0.4 }} />
                                    ...
                                    <ListItem
                                        ...
                                        sx={{
                                            ...
                                            bgcolor: index === selectedIndex
                                                ? farmaColors.alpha?.primary30 || "rgba(204,108,6,0.15)"
                                                : "white",
                                            "&:hover": { bgcolor: farmaColors.alpha?.primary20 || "rgba(204,108,6,0.1)", cursor: "pointer" },
                                            py: 1.2,
                                            px: 2,
                                        }}
                                    >
                                        <Box sx={{ display: "flex", alignItems: "center", gap: 2, width: "100%" }}>
                                            ...
                                            <Box sx={{ flex: 1, minWidth: 0 }}>
                                                <Typography variant="body2" sx={{ fontWeight: 600, fontSize: "0.84rem", color: "#1a1a1a" }}>
                                                    {product.producto}
                                                    {product.presentacion && ` | ${product.presentacion}`}
                                                    {product.laboratorio && ` | ${product.laboratorio}`}
                                                </Typography>
                                                <Typography variant="caption" sx={{ fontSize: "0.7rem", color: "text.secondary" }}>
                                                    Lote: {product.numeroLote || "S/N"} | Vence: {product.fechaVencimiento ? product.fechaVencimiento.split('T')[0] : "---"}
                                                </Typography>
                                            </Box>
                                            ...
                                            <Chip
                                                ...
                                                size="small"
                                                sx={{
                                                    bgcolor: (product.stockProducto ?? 0) > 0 ? "#4CAF50" : "#f44336",
                                                    color: "white",
                                                    height: 24,
                                                    fontSize: "0.72rem",
                                                    fontWeight: 700,
